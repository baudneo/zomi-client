...
class PipeLine:
    ...
    async def image_generator(self):
        ...
        logger.debug(
            f"{LP}image_generator: STARTING {self.frames_attempted = } ---- {self.total_max_frames = } ::: {self.frames_attempted < self.total_max_frames = }"
        )
    ...
    **nph-zms is symlinked to zms**
    ...
    async def get_image(self) -> Tuple[Optional[Union[bytes, bool]], Optional[str]]:
        ...
        for image_grab_attempt in range(self.max_attempts):
            ...
            logger.debug(
                f"perf:{lp} ZMS request took: {time.perf_counter() - _perf:.5f}"
            )
            ...
            if not api_response:
                ...
                if not g.past_event:
                    ...
                    if self.event_ended:  # Assuming event has ended
                        logger.debug(
                            f"{lp} event has ended, checking OOB status... {self.current_frame = } -- {self.event_tot_frames = } -- {self.current_frame > self.event_tot_frames = }"
                        )
                        # is current frame OOB
                        if self.current_frame > self.event_tot_frames:
                            # We are OOB, so we are done
                ...
                logger.debug(
                    f"{lp} response is not bytes -> {type(api_response) = } -- {api_response = }"
                )
